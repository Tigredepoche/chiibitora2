...
function handleData(owo) {
    ...
    let info = owo.data.Media
    let listegenres = info.genres
    let toutlesgenres = listegenres.join()
    
        //Car tout est souvent ongoing :(
    if(info.endDate.day === null){
        let ENDDATE = 'Still releasing'
    }
    else {
        let ENDDATE = info.endDate.day + '/' + info.endDate.month + '/' + info.endDate.year
    }
    ...
    let striphere = 1021
    let shortText = text.substring(0, striphere) + '...'
    //Et parfois les balises html me tilt dans les descriptions z.z
    let descSTR = shortText
    let descSTRembed = descSTR.replace(/<br *\/?>/g, "\n").replace(/\n|&rsquo;/,'');
...
//       console.log(info.streamingEpisodes[0].title)
    let titre = info.streamingEpisodes[0];
if(titre === undefined ){ 
} else {
